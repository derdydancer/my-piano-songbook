...
import { ArrowUpOnSquareIcon, ArrowDownOnSquareIcon, SunIcon, MoonIcon, InformationCircleIcon, PlusCircleIcon, TrashIcon, PencilIcon } from '../../components/common/Icons';
import { useTheme } from '../../contexts/ThemeContext';
import { geminiService } from '../../services/geminiService';
import { DEFAULT_PLATE_DENOMINATIONS, ALL_LIFTS, BAR_WEIGHT, UTILITY_IDS } from '../../constants';
...
const SettingsPage: React.FC = () => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Add more robust validation here if necessary, e.g. check for specific properties
          if (parsedData && parsedData.utilitySettings && parsedData.weightEntries && parsedData.disneyCollection && parsedData.giftRecipientLists && parsedData.plateInventory && parsedData.exerciseSettings && parsedData.workoutSessions) {
            if (window.confirm("Importing data will overwrite existing data. Are you sure?")) {
          ...
          alert(`Failed to import data. Please ensure the file is a valid JSON backup. Error: ${(error as Error).message}`);
        } finally {
  ...
  const handleSavePlate = () => {
    ...
    if (editingPlate) {
      ...
      if (plateInventory.some(p => p.denomination === currentPlateDenom)) {
        alert("This plate denomination already exists in your inventory. Edit the existing entry instead.");
        return;
      }
  ...
  const handleDeletePlate = (denomination: number) => {
    if (window.confirm(`Are you sure you want to remove ${denomination}kg plates from your inventory?`)) {
  ...
  const trainUtility = getUtilitySetting(UTILITY_IDS.TRAIN as UtilityId);


  return (
    <div className="p-4 space-y-6 mb-16"> {/* Added mb-16 for bottom nav clearance */}
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="mt-2 text-xs text-textSecondary">
              Export/Import your app data as a JSON file. Importing overwrites current data.
            </p>
          </div>
          <div>
            ...
            <div className="mt-4">
              ...
              <div className="flex space-x-2">
                <Input
                  ...
                  placeholder="Enter Gemini API Key (Dummy feature)"
                  className="w-full"
                />
      ...
      {utilitySettings.map(utility => (
        <CollapsibleSection key={utility.id} title={`${utility.name} Settings`}>
          <div className="space-y-4">
            ...
            {/* Specific settings for Workout Tracker (Train) */}
            {utility.id === UTILITY_IDS.TRAIN && utility.enabled && (
                            ...
                            {plateInventory.sort((a,b) => b.denomination - a.denomination).map(plate => (
                                <li key={plate.denomination} className="flex justify-between items-center p-2 bg-background dark:bg-gray-700 rounded">
                                    ...
                                    <div className="space-x-1">
                                        <Button variant="ghost" size="sm" onClick={() => handleOpenEditPlateModal(plate)}><PencilIcon className="w-4 h-4"/></Button>
                                        <Button variant="ghost" size="sm" onClick={() => handleDeletePlate(plate.denomination)}><TrashIcon className="w-4 h-4 text-red-500"/></Button>
                                    </div>
                                </li>
                            ))}
                        </ul>
                    )}
                    <p className="mt-2 text-xs text-textSecondary">Bar is assumed to be {BAR_WEIGHT}kg.</p>
                </div>
                <div>
            ...
            {/* Add specific settings for other utilities here if needed */}
          </div>
        </CollapsibleSection>
      ))}
